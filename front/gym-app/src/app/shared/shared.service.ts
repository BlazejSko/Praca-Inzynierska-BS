...
export class SharedService {
  ...
  getClassroomsList(): Observable<Classroom> {
    return this.http
      .get<PaginatedList<Classroom>>(this.APIUrl + '/classrooms')
  ...
  getClassCategoryList(): Observable<ClassesCategory> {
    return this.http
      .get<PaginatedList<ClassesCategory>>(this.APIUrl + '/classescategory')
      .pipe(map((data: PaginatedList<ClassesCategory>) => data.results));
  }

  getOneClassCategory(id: number): Observable<ClassesCategory> {
    return this.http.get<ClassesCategory>(
      this.APIUrl + '/classescategory/' + id
    );
  }
  ...
  getClassesList(): Observable<Classes> {
    return this.http
      .get<PaginatedList<Classes>>(this.APIUrl + '/classes')
      .pipe(map((data: PaginatedList<Classes>) => data.results));
  }

  getSingleClass(class_id: number): Observable<UserClasses> {
    return this.http.get<UserClasses>(this.APIUrl + '/classes/' + class_id);
  }
  ...
  getUserClassesList(): Observable<UserClasses> {
    return this.http
      .get<PaginatedList<UserClasses>>(this.APIUrl + '/userclasses')
  ...
  getLoggedinUser() {
    return this.http.get('http://127.0.0.1:8000/gymapp/user', {
      withCredentials: true,
    });
  }
  ...
  // addUserClassesForGivenUser(id: number){
